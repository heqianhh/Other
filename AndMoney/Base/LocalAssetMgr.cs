...
namespace AndMoney {
    ...
    public class LocalAssetMgr : Singleton<LocalAssetMgr> {
        ...
        public GameObject Get_Prefab(string name) {
            return GameObject.Instantiate(Load_Prefab(name));
        }
        ...
        public Material Load_Material(string name) {
            Material prefab = null;
            string subPath = "Materials/";
            string path = subPath + name;
            prefab = Resources.Load(path) as Material;
            if (prefab == null) {
                Debug.LogError("Failed Load_Material from " + path);
            }
            return prefab;
        }
        public PhysicMaterial Load_PhysicsMaterial(string name) {
            PhysicMaterial prefab = null;
            string subPath = "PhysicsMaterials/";
            string path = subPath + name;
            prefab = Resources.Load(path) as PhysicMaterial;
            if (prefab == null) {
                Debug.LogError("Failed Load_PhysicsMaterial from " + path);
            }
            return prefab;
        }
        ...
        public ScriptableObject LoadScriptableObject(string name) {
            ScriptableObject scriptableObject = null;
            string subPath = "ScriptableObjects/";
            string path = subPath + name;
            scriptableObject = Resources.Load(path) as ScriptableObject;
            if (scriptableObject == null) {
                Debug.LogError("Failed LoadScriptableObject from " + path);
            }
